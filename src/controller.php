...
class Controller {
	...
	private function beforeRequest()
	{
		...
		if(isset($this->_behaviors['access']))
		{
			...
			if(isset($this->_behaviors['access']['rules']['isLogged']) && $this->_behaviors['access']['rules']['isLogged'] == 'true')
			{
				...
				if(isset($resp['Error']))
				{
					(isset(goat::$app->config['display_login_error']) && goat::$app->config['display_login_error'] == 'true') ? goat::$app->setFlash("_login_error", $resp) : '';
