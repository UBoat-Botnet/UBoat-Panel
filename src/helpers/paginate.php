    ...
    public function dataview($query, $params = []) //$removeFirst = false, $valueIndex = -1, $valueTag = "value", $preTags = ["<td>", "</td>"]
    {
        ...
        if ($stmt->rowCount() > 0) {
            ...
            while ($row = $stmt->fetch(PDO::FETCH_ASSOC)) {
                ...
                {
                    $row[$params['stumble']] = (($row[$params['stumble']] == 1) ? $params['stumbleData'][2]."Online".$params['stumbleData'][3] : $params['stumbleData'][0]."Offline".$params['stumbleData'][1]);
                }
                if(isset($params['flags']) && $params['flags'] == 'true')
                {
